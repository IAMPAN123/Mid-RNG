...
halfcursorwidth = cursor.get_width()/2
halfcursorheight = cursor.get_height()/2
ps = printslider()
testupgimg = pygame.image.load('Images/placeholder.png').convert_alpha()
testupg = Circle(500, 50, testupgimg, width = 50, height = 50)
testupg2 = Circle(500, 500, testupgimg, width = 50, height = 50)
...
while running:
    ...
    if scrnstate == None:
        ...
        #self.ps.run()
        if testupg.draw(scrn):
            scrnstate == 'circle'

        #self.newtimer = pygame.time.get_ticks()
        #if self.newtimer >= 5000:
            #if self.testupg.draw():
                #print('work')
    ...
    scrn.blit(cursor, (mousepos[0] - halfcursorheight, mousepos[1] - halfcursorwidth))
